...
export const SubagentToolInputSchema = z.preprocess(
  ...
  z.object({
    ...
    thinking_level: z
      ...
      .optional()
      .describe(
        'Optional per-call thinking effort level for this subagent turn ("off", "low", "medium", "high", "xhigh", "max"). Overrides the subagent profile and parent session default. Ignored when resuming — resumed subagents keep their own thinking level.',
      ),
    model: z
      .string()
      .trim()
