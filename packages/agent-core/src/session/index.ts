...
export class Session {
  ...
  async createMain() {
    ...
    // Custom `--agents-dir` profiles take precedence over the agentfile
    // catalog; otherwise `--agent` / `--agent-file` / defaults apply.
    const profile =
      this.options.profiles?.['agent'] ??
      resolveMainAgentProfile(this.agentCatalog, this.options.agents?.profileName);
  ...
  async resume(): Promise<{ warning?: string }> {
    ...
    const profile = this.options.profiles?.['agent'] ?? this.agentCatalog.getDefault();
  ...
  *readyAgents(): Iterable<Agent> {
  ...
  private resolvePersistedProfile(
    ...
    // Fall through to the agentfile catalog when no custom profiles are set.
    const profiles = this.options.profiles;
    if (meta.type === 'sub') {
      const parentProfileName = parentAgent?.config.profileName ?? 'agent';
      return (
        profiles?.[parentProfileName]?.subagents?.[profileName] ??
        profiles?.['agent']?.subagents?.[profileName] ??
        this.agentCatalog.delegatableSubagents(parentProfileName)[profileName]
      );
    }
    return profiles?.[profileName] ?? this.agentCatalog.get(profileName);
  }
